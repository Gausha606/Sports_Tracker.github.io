...
const App = () => {
  ...
  const resetAll = () => {
    if (window.confirm("Naya match start karein? Purana data clear ho jayega.")) {
      localStorage.clear();
  ...
  const saveInningsStats = (currentPlayers, currentStats) => {
    const key = innings === 1 ? 'inn1Batting' : 'inn2Batting';
    const playedThisInns = currentPlayers.filter(p => p.balls > 0 || p.isOut);
    setStats(prev => ({ ...prev, [key]: playedThisInns }));
  };

  const checkMatchStatus = (currentMatch, currentPlayers, currentStats) => {
    if (innings === 1 && currentMatch.completedOvers >= maxOvers) {
      saveInningsStats(currentPlayers, currentStats);
      ...
      if (currentMatch.totalRuns >= target) {
        ...
        saveInningsStats(currentPlayers, currentStats);
        if (currentMatch.totalRuns < target - 1) setWinner("First Innings Won! 🏆");
        else if (currentMatch.totalRuns === target - 1) setWinner("Match Tied! 🤝");
      }
    }
  };
          ...
          <tbody>
            {batData.map((p, i) => (
              <tr key={i} className="border-t border-slate-800">
                ...
                <td className="p-3 text-blue-400">{p.runs}</td>
                <td className="p-3">{p.balls}</td>
                <td className="p-3 text-orange-400">{p.fours}</td>
                <td className="p-3 text-green-400">{p.sixes}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <tbody>
            {Object.values(bowlData).map((b, i) => (
              <tr key={i} className="border-t border-slate-800">
                ...
                <td className="p-3">{formatOvers(b.balls)}</td>
                <td className="p-3 text-red-400">{b.runs}</td>
                <td className="p-3 text-orange-500 font-black">{b.wickets}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 font-sans flex flex-col items-center">
      ...
      {step === 'setup' && (
        <div className="w-full max-w-md bg-slate-900 p-8 rounded-3xl border border-slate-800 mt-10 shadow-2xl">
          ...
          <div className="space-y-5">
            <div>
              <label className="text-xs font-bold text-slate-500">MAX OVERS</label>
              <input type="number" className="w-full p-4 bg-slate-800 rounded-xl mt-1 border border-slate-700 outline-none focus:border-blue-500" value={maxOvers} onChange={(e)=>setMaxOvers(Number(e.target.value))} />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500">PLAYERS (COMMA/ENTER)</label>
              <textarea className="w-full p-4 bg-slate-800 rounded-xl mt-1 h-48 border border-slate-700 outline-none" placeholder="Virat, Dhoni..." value={playerInput} onChange={(e)=>setPlayerInput(e.target.value)} />
            </div>
            <button onClick={initMatch} className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-2xl font-black uppercase shadow-lg shadow-blue-900/20 transition-all">Start Match</button>
          </div>
        </div>
      )}
      ...
      {step === 'match' && !showReport && (
        <div className="w-full max-w-lg mt-8">
          <div className="flex justify-between items-end mb-6 px-2">
            <div>
              <p className="text-xs font-black text-blue-500 uppercase">Innings {innings}</p>
              <h1 className="text-7xl font-black">{match.totalRuns}<span className="text-3xl text-slate-600">/{match.wickets}</span></h1>
            </div>
            {target && (
              <div className="bg-amber-600/10 border border-amber-500 p-3 rounded-2xl text-right">
          ...
          <div className="bg-slate-900 rounded-3xl p-5 border border-slate-800 mb-4 shadow-xl">
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/40 p-4 rounded-2xl border border-blue-500/20">
                   ...
                   <p className="text-2xl font-black text-blue-400">{getPlayer(match.strikerId).runs} <span className="text-xs text-slate-500 font-normal">({getPlayer(match.strikerId).balls})</span></p>
                </div>
                <div className="bg-slate-800/40 p-4 rounded-2xl border border-slate-800">
                   ...
                   <p className="text-2xl font-black text-slate-200">{getPlayer(match.nonStrikerId).runs} <span className="text-xs text-slate-500 font-normal">({getPlayer(match.nonStrikerId).balls})</span></p>
                </div>
             </div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[0, 1, 2, 3].map(n => <button key={n} onClick={()=>handleEntry(n)} className="h-16 bg-slate-800 rounded-2xl font-black text-xl hover:bg-slate-700 shadow-md">{n}</button>)}
            ...
            <button onClick={()=>setMatch(p=>({...p, totalRuns: p.totalRuns+1}))} className="h-16 bg-slate-900 rounded-2xl font-bold text-blue-400 border border-blue-900/50 text-xs">WD</button>
            <button onClick={()=>setMatch(p=>({...p, totalRuns: p.totalRuns+1}))} className="h-16 bg-slate-900 rounded-2xl font-bold text-blue-400 border border-blue-900/50 text-xs">NB</button>
            <button onClick={handleWicket} className="col-span-2 h-16 bg-red-600 rounded-2xl font-black text-xl shadow-lg uppercase">Wicket</button>
      ...
      {showReport && (
        <div className="w-full max-w-lg mt-4 animate-in fade-in slide-in-from-bottom-4">
          ...
          <button onClick={resetAll} className="w-full bg-white text-black py-5 rounded-3xl font-black text-lg mb-10 shadow-2xl">START NEW TOURNAMENT</button>
        </div>
      )}

      {/* Modals Same as before with minor tweaks for reporting */}
      {showModal && (
          ...
          <div className="bg-slate-900 w-full max-w-sm rounded-3xl p-6 border border-slate-800 shadow-2xl">
            ...
            {showModal === 'initial-selection' && (
              <div className="space-y-4">
                ...
                <button onClick={() => {
                  if(match.strikerId === null || match.nonStrikerId === null || !match.bowler) return alert("Select all fields!");
                  setShowModal(null);
            ...
            {showModal === 'over-end' && (
              <div className="grid gap-2">
                {players.filter(p => p.name !== match.lastBowler && p.name !== match.bowler).map(p => (
                  <button key={p.id} onClick={()=>{setMatch({...match, lastBowler: match.bowler, bowler: p.name}); setShowModal(null);}} className="p-4 bg-slate-800 rounded-xl font-bold">🏏 {p.name}</button>
                ))}
              </div>
            )}
            ...
            {showModal === 'new-batsman' && (
              <div className="grid gap-2">
                {players.filter(p => !p.isOut && p.id !== match.nonStrikerId).map(p => (
                  <button key={p.id} onClick={()=>{setMatch({...match, strikerId: p.id}); setShowModal(null);}} className="p-4 bg-slate-800 rounded-xl font-bold">👤 {p.name}</button>
                ))}
              </div>
            )}
            ...
            {showModal === 'innings-break' && (
              <div className="text-center py-4">
                 <p className="text-4xl font-black mb-8 tracking-tighter">TARGET: {target}</p>
                 <button onClick={() => {
            ...
            {showModal === 'adjust' && (
               <button onClick={()=>setShowModal(null)} className="w-full bg-blue-600 py-3 rounded-xl font-black mt-4">Close Adjust</button>
            )}
          </div>
        </div>
      )}
      ...
      {/* Result Screen */}
      {winner && !showReport && (
        <div className="fixed inset-0 bg-blue-700 z-100 flex flex-col items-center justify-center p-8 text-center animate-in zoom-in-95">
          ...
          <div className="space-y-4 w-full max-w-xs">
            ...
            <button onClick={resetAll} className="w-full bg-black text-white py-5 rounded-3xl font-black text-lg">NEW TOURNAMENT</button>
          </div>
        </div>
      )}
    </div>
